...
def load_data(file_path):
    """Load data from a CSV file, automatically removing metadata."""
    with open(file_path, 'r') as f:
        lines = f.readlines()
        
    # Find the first line that contains the header
    for i, line in enumerate(lines):
        if 't(s)' in line:  # Assume 't(s)' is a key column in the actual data
            header_index = i
            break
    
    # Read the CSV from the header line
...
if 'file_path' in st.session_state:
    ...
    if os.path.exists(file_path):
        ...
        if not df.empty:
            ...
            st.write('## Matplotlib Plots')

            # Plot t(s) vs I(A)
            fig1, ax1 = plt.subplots()
            if 't(s)' in df.columns and 'I(A)' in df.columns:
                ax1.plot(df['t(s)'], df['I(A)'], label='Current vs Time')
                ax1.set_title(f'Current vs Time from {file_name}')
                ax1.set_xlabel('Time (s)')
                ax1.set_ylabel('Current (A)')
                ax1.legend()
                st.pyplot(fig1)
            else:
                st.write("No recognized columns for plotting Current vs Time.")

            # Plot t(s) vs Ewe(V)
            fig2, ax2 = plt.subplots()
            if 't(s)' in df.columns and 'Ewe(V)' in df.columns:
                ax2.plot(df['t(s)'], df['Ewe(V)'], label='Potential vs Time')
                ax2.set_title(f'Potential vs Time from {file_name}')
                ax2.set_xlabel('Time (s)')
                ax2.set_ylabel('Potential (V)')
                ax2.legend()
                st.pyplot(fig2)
            else:
                st.write("No recognized columns for plotting Potential vs Time.")
